@Login
Feature: Test Login and Account Management

  Background:
    Given <actor> go to bml login page

  @TC_LOG_03 @LoginSuccess
  Scenario Outline: Login success with role user
  ...
  Scenario Outline: Login success with role letan
    When <actor> logs in with username "<username>" and password "<password>"
    Then the user should see the profile name as "<profileName>"
  ...
  Scenario Outline: Login success with role admin
    When <actor> logs in with username "<username>" and password "<password>"
    Then the user should see the profile name as "<profileName>"
